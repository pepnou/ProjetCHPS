...
void Mandelbrot::escapeSpeedCalc()
{
	...
	/*int args[this->im_height*this->surEchantillonage];
	pthread_t tid[this->im_height*this->surEchantillonage];
	
	int verif;
	for( int j = 0; j < this->im_height*this->surEchantillonage; ++j)
	{
		args[j]=j;
		verif = pthread_create( &(tid[j]), nullptr, this->threadCalc, &(args[j]));
		if(verif)
			exit(1);
	}
	...
	for( int j = 0; j < this->im_height*this->surEchantillonage; ++j)
	{
		verif = pthread_join( tid[j], nullptr);
		if(verif)
			exit(2);
	}*/
...
/*void* Mandelbrot::threadCalc(void* arg)
{
	int* j = (int*)arg;
